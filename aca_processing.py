...
def parse_date_safe(d, default_end: bool = False):
    ...
    try:
        if len(s) == 4 and s.isdigit():
            y = int(s)
            return date(y, 12, 31) if default_end else date(y, 1, 1)
        if len(s) == 7 and s[4] == "-":
            y, m = map(int, s.split("-"))
...
def load_excel(file_bytes: bytes) -> dict:
    ...
    for raw in xls.sheet_names:
        ...
        # CSV-in-a-single-column fallback
        if df.shape[1] == 1:
...
def _ensure_employeeid_str(df):
    if df.empty or "employeeid" not in df.columns:
        return df
    df = df.copy()
    df["employeeid"] = df["employeeid"].map(_normalize_employeeid)
    return df
...
def prepare_inputs(data: dict):
    ...
    for sheet, cols in EXPECTED_SHEETS.items():
        ...
        if df.empty:
            cleaned[sheet] = pd.DataFrame(columns=cols)
            continue
...
def _grid_for_year(employee_ids, year: int) -> pd.DataFrame:
    ...
    for emp in employee_ids:
        for m in range(1, 13):
            ...
            recs.append({
                "employeeid": emp, "year": year, "monthnum": m, "month": ms.strftime("%b"),
