...
# ---------- Small helpers to avoid ambiguous truth ----------
def is_df_present(df) -> bool:
    """Safely check if a DataFrame is present and non-empty."""
    return isinstance(df, pd.DataFrame) and (not df.empty)

def is_series_nonempty_bool(s: pd.Series) -> bool:
    """Safely decide if a boolean Series contains at least one True."""
...
def build_interim_df(year: int, excel_bytes: bytes) -> pd.DataFrame:
    ...
    def compute_line_codes(r):
        ...
        waitp   = (r.get("is_waiting_period_true_full_month") == "Yes")
