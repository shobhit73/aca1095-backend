...
def _overlap_full_month(start: pd.Timestamp, end: pd.Timestamp, ms: pd.Timestamp, me: pd.Timestamp) -> bool:
    """
...
def _overlap_any(start: pd.Timestamp, end: pd.Timestamp, ms: pd.Timestamp, me: pd.Timestamp) -> bool:
    """
...
def _any_overlap(df: pd.DataFrame, start_col: str, end_col: str, ms: pd.Timestamp, me: pd.Timestamp) -> bool:
    if df.empty:
        return False
...
def _all_month(df: pd.DataFrame, start_col: str, end_col: str, ms: pd.Timestamp, me: pd.Timestamp) -> bool:
    if df.empty:
        return False
    for _, r in df.iterrows():
...
def _union_intervals_full_month(df: pd.DataFrame, start_col: str, end_col: str, ms: pd.Timestamp, me: pd.Timestamp) -> bool:
    """
...
def _value_latest_overlap(df: pd.DataFrame, start_col: str, end_col: str, ms: pd.Timestamp, me: pd.Timestamp, value_col: str) -> Optional[float]:
    """
    ...
    # Sort by start date descending to pick the last starting interval
...
@dataclass
class MonthWindow:
    ym: Tuple[int, int]  # (year, month_int)
    start: pd.Timestamp
    end: pd.Timestamp
...
def build_interim(
    ...
    def month_windows(y: int) -> List[MonthWindow]:
        ...
        for name, m in MONTHS:
            ...
            if m == 12:
                me = pd.Timestamp(year=y, month=12, day=31)
            else:
                me = pd.Timestamp(year=y if m < 12 else y, month=m + 1, day=1) - pd.Timedelta(days=1)
    ...
    for emp_id, demo_rows in demo_by_emp.items():
        ...
        for win in month_windows(year):
            ...
            # ---- employment present any day in month
            employed_any = _any_overlap(st_emp, "statusstartdate", "statusenddate", ms, me)

            # Terminated this month? mark not employed_any for Line 16 later
            ...
            if not st_emp.empty:
                for _, r in st_emp.iterrows():
                    ...
                    if role in ("TERM", "TERMINATED"):
                        # If termination date overlaps month, mark it.
            ...
            if not st_emp.empty:
                ft_rows = st_emp[
                    st_emp.apply(
                        lambda r: _norm(r.get("role", r.get("employmenttype", ""))) in ("FT", "FULLTIME"),
                        axis=1,
                    )
                ]
                pt_rows = st_emp[
                    st_emp.apply(
                        lambda r: _norm(r.get("role", r.get("employmenttype", ""))) in ("PT", "PARTTIME"),
                        axis=1,
                    )
                ]
            ...
            # ---- eligibility rows (employee/spouse/child offers)
            ...
            spouse_elig_rows = elig_emp[elig_emp["tier"].isin(["EMPSPOUSE", "SPOUSE", "EMP+SPOUSE", "EMP_SPOUSE", "EMP-SPOUSE", "EMPFAM"])].copy()
            child_elig_rows = elig_emp[elig_emp["tier"].isin(["EMPCHILD", "CHILD", "EMP+CHILD", "EMP_CHILD", "EMP-CHILD", "EMPFAM"])].copy()

            # ---- MV offer (PlanA for the full month in allowed tiers)
            ...
            # ---- employee offer (full month any tier of EMP in eligibility)
            ee_offer_full = _all_month(ee_elig_rows, "eligibilitystartdate", "eligibilityenddate", ms, me)

            # ---- union enrollment for EMP tier (full month)
            en_emp_work = en_emp.copy()
            en_emp_work["tier"] = en_emp_work.get("enrollmenttier", "").map(_norm)

            def _tier_enrolled_full_month(df: pd.DataFrame, tier_options: Tuple[str, ...], ms: pd.Timestamp, me: pd.Timestamp) -> bool:
                if df.empty:
                    return False
                ...
                if work.empty:
                    return False
                # If isenrolled col exists and false, ignore those rows
                if "isenrolled" in work.columns:
                    work = work[work["isenrolled"].fillna(True) == True]
                    if work.empty:
                        return False
                # check union full-month
                return _union_intervals_full_month(work, "enrollmentstartdate", "enrollmentenddate", ms, me)

            enrolled_full = _tier_enrolled_full_month(en_emp_work, ("EMP", "EMPFAM", "EMPSPOUSE", "EMPCHILD"), ms, me)

            # ---- spouse/child eligibility (FULL MONTH in Eligibility tiers)
            spouse_eligible = _all_month(
                spouse_elig_rows,
                "eligibilitystartdate", "eligibilityenddate", ms, me,
            )
            child_eligible = _all_month(
                child_elig_rows,
                "eligibilitystartdate", "eligibilityenddate", ms, me,
            )
            ...
            # If the family tier (EMPFAM) is enrolled for the full month in any plan,
            # then both spouse and child are considered enrolled for that month.
            ...
            # Pick latest overlapping eligibility EMP row and read plancost
            ...
            # ---- waiting period: employed but no eligibility yet and a future elig start exists
            ...
            # ---- Line 14 logic
            line14 = ""
            # Special guardrail: if enrolled full-month but no eligibility rows in month, treat as 1E
            ...
            if enrolled_full and not have_elig_now_any_tier:
                ...
                if not ee_offer_full and not mv_offer_full:
                    ...
                    # We have an offer; decide along MV path if applicable
                    if mv_offer_full:
                        # MV path
                        if offer_spouse and offer_dependents:
                            # spouse + dependents
                            # 1A if affordable, else 1E
    ...
    # compute employee-level 1G
    # If an employee is NEVER FT in any month AND enrolled at least one full month -> 1G across all months
    g_rows = []
    for emp_id, sub in interim.groupby("employeeid"):
        never_ft = not sub["ft_all"].any()
        enrolled_any = sub["enrolled_full"].any()
        line14_all12 = ""
        if never_ft and enrolled_any:
            line14_all12 = "1G"
    ...
    # If 1G set, blank out monthly line14 in the interim (keep it visible as empty)
...
def build_final(interim: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    # add All-12 columns by employee
    agg = interim.groupby("employeeid").agg(
        line14_all12=("line14_all12", "first")
    ).reset_index()

    # derive All-12 identical values for line14/line16 when applicable
...
def build_penalty_dashboard(interim: pd.DataFrame, year: int) -> pd.DataFrame:
    """
    Build a simple penalty dashboard:
      - Penalty A if no MEC offer (offer_employee_full=False)
      - Penalty B if MEC offered but not enrolled and unaffordable
      Otherwise '-'
    """
    rows: List[Dict] = []
    for emp_id, sub in interim.groupby("employeeid"):
        ...
        for _, r in sub.iterrows():
            ...
            if not offer:
                # No offer -> Penalty A unless not employed or waiting period
                if employed and not waiting:
                    totals[m] = PENALTY_A_MONTHLY
                    reasons[m] = "No MEC offer"
                elif waiting:
                    totals[m] = "-"
                    reasons[m] = "Waiting period"
                else:
                    totals[m] = "-"
                    reasons[m] = "Not employed"
            else:
                # Offered MEC
                if enrolled:
                    totals[m] = "-"
                    ...
                    if affordable:
                        totals[m] = "-"
                        reasons[m] = "Affordable offer declined"
                    else:
                        totals[m] = PENALTY_B_MONTHLY
                        reasons[m] = "Unaffordable + not enrolled"
