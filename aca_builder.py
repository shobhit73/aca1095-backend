# aca_builder.py
# 2) Interim/Final & Penalty Dashboard logic (the only place you should change rules)
...
from aca_processing import (
    ...
    _status_from_demographic, _any_overlap, _all_month, MONTHS, FULL_MONTHS, MONTHNUM_TO_FULL
)

# ----- Role tokens & “employed” semantics -----
...
def build_interim(emp_demo, emp_status, emp_elig, emp_enroll, dep_enroll, year=None, pay_deductions=None) -> pd.DataFrame:
    """
    Produces the month-by-month Interim table with all flags + Line 14/16.
    Business rules here only.
    """
    if year is None:
    ...
    if (stt is None) or stt.empty or not {"statusstartdate","statusenddate"} <= set(stt.columns):
        ...
        from aca_processing import _normalize_employeeid, _parse_date_cols
    ...
    for _,row in out.iterrows():
        ...
        # ----- Employed (any overlap of ACTIVE/LOA/FT/PT) -----
        employed=False
        ...
        if not el_emp.empty and {"eligibilitystartdate","eligibilityenddate"} <= set(el_emp.columns):
            ...
            # MV if eligible for PlanA (any overlap is enough per your spec)
            ...
            # Dependents eligibility from eligibility tier
        ...
        # Dep Enrollment can also prove dependent enrollment (ignoring waivers)
        ...
        # ----- Affordability from EMP-tier cost (<$50) -----
        ...
        flags.append({
            ...
            "eligible_mv": eligible_mv,                 # TRUE if eligible for PlanA at any time in month
            "affordable_plan": affordable,              # NEW: explicit column for affordability (<$50 EMP)
    ...
    flag_cols = [
        ...
        "eligibleforcoverage","eligible_allmonth","eligible_mv","affordable_plan","offer_ee_allmonth",
...
def build_final(interim: pd.DataFrame) -> pd.DataFrame:
    """
    Returns the Final table with Line 14 & Line 16 by month (sorted Jan..Dec).
    """
    df = interim.copy()
    if "monthnum" in df.columns:
        df["monthnum"] = df["monthnum"].apply(lambda x: _safe_int(x, x))  # NaN-safe
...
# ----- Penalty Dashboard -----
PENALTY_A = 241.67  # 1H
PENALTY_B = 362.50  # 1E & NOT enrolled_allmonth
...
def build_penalty_dashboard(interim: pd.DataFrame,
    ...
    """
    Wide table: EmployeeID, Reason, January..December
      - Penalty A: Line14 == '1H'
      - Penalty B: Line14 == '1E' AND NOT enrolled_allmonth
    """
    if interim.empty:
    ...
    for emp, g in df.groupby("EmployeeID", sort=True):
        ...
        for _, r in g.iterrows():
            mfull = r.get("MonthFull")
            if pd.isna(mfull):
                continue
            months_map[mfull] = _money(float(r["_pen_amt"])) if r["_pen_amt"] else "-"
        has_B = (g["_pen_type"]=="B").any()
        has_A = (g["_pen_type"]=="A").any()
        reason = _PENALTY_TEXT_B if has_B else (_PENALTY_TEXT_A if has_A else "")
        if has_A:
            wait_months = g.loc[g["waitingperiod_month"] & (g["_pen_type"]=="A"), "MonthFull"].dropna().tolist()
            if wait_months:
